...
<script language="javascript">
	...
	{			
		var form=document.getElementById("ProInco");
		form.action="${pageContext.request.contextPath}/errorDisc_toproIncoPage.action?tnum=${user.tnum}";
		form.submit();  
	}
	</script>
...
<body>
	<div class="right_cont">
		<div class="title_right">
			<strong>专业信息不完整</strong>
		</div>
		<div></div>
		<div id="box2">
			<form method="post" id="ProInco">
				<td>学院：</td>
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<thead>
						<tr align="center">
							...
							<th nowrap="nowrap"><small>尚未填写的信息</small>
							</td>
						</tr>
					</thead>
					<c:if test="${departmentEmptyListCount > 0}">

						<c:forEach items="${departmentEmptyList}" var="departmentEmpty"
							varStatus="xh">
							<tr align="center">
								<td nowrap="nowrap"><small>${departmentEmpty.college.collegeCname }</small></td>
								<td nowrap="nowrap"><small>${departmentEmpty.departmentCname }</small></td>
								<td nowrap="nowrap"><c:forEach
										items="${infoEmptyList[xh.count-1]}" var="infoEmpty">
										<small style="color: red;">${infoEmpty } </small>
									</c:forEach></td>
							</tr>
						</c:forEach>
					...
					<c:if test="${departmentEmptyListCount == 0}">
						<tr align="center">
							<th nowrap="nowrap" colspan="9"><small>没有专业信息不完整的专业！</small>
							</th>
						</tr>
					</c:if>
					</tbody>
