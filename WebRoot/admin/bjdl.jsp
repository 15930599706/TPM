					...
					if(ajax.readyState == 4){					 
						if(ajax.status == 200){					
							...
							for(var i=0;i<size;i++){
								...
								if(str[0] == '${users.department.departmentid}')	//设置专业的默认选定值
								{
									option.selected = true;
								}   
								departmentElement.appendChild(option);
									
								if(i == (size-1))	//设置“其他”选项，用于设置没有专业的老师
								{
									var option = document.createElement("option");
									option.value = '-1';
									option.innerHTML = '其他'; 
									if('${users.department.departmentid}' == '')
									{
										option.selected = true;
									}
									departmentElement.appendChild(option); 
...
<body
	<c:if test="${user.adminlevel == 5 || user.adminlevel == 3}">onload="xueyuanSelect()"</c:if>>
	<div class="right_cont">
		...
		<div style="width: 900px; margin: auto;">
			<form
				action="${pageContext.request.contextPath}/user_updateuser.action"
				...
				<table class="table table-bordered">
					...
					<tr>
						...
						<td><input type="text" name="username" class="span4"
							value="${users.username }" required /></td>
					</tr>
					<tr>
						<td width="20%" align="right" nowrap="nowrap" bgcolor="#f1f1f1">性别：</td>
						<td><input name="sex" type="radio" value="男"
							<c:if test="${users.sex eq '男' }">checked="checked"</c:if> />男 <input
							name="sex" type="radio" value="女"
							<c:if test="${users.sex eq '女' }">checked="checked"</c:if> />女</td>
					</tr>
					<tr>
									...
									onChange="xueyuanSelect()" size="1">
										<c:forEach items="${collegelist}" var="college">
											<option value="${college.collegeid }"
												<c:if test="${users.college.collegeid == college.collegeid }">selected="selected"</c:if>>${college.collegeCname }</option>
										</c:forEach>
								</select></td>
							</tr>
							<tr>
								...
								<td><select name="department.departmentid" id="xi" size="1">
										<c:forEach items="${departmentlist}" var="department">
											<option value="${department.departmentid }"
												<c:if test="${users.department.departmentid == department.departmentid }">selected="selected"</c:if>>${department.departmentCname }</option>
										</c:forEach>
								</select></td>
							</tr>
						</c:if>
						...
						<input type="hidden" name="adminlevel"
							value="${users.adminlevel }" />
						<input type="hidden" name="tnum" value="${users.tnum }" />
					<tr>
