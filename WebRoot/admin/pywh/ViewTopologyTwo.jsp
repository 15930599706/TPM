...
<body>
	<div class="right_cont">
		...
		<c:if test="${user.adminlevel == 1 || tag == 'revise'}">
			<div id="box1">
				<c:if test="${user.adminlevel == 1}">
					<a
						href="topology_changetag.action?department.departmentid=${department.departmentid }">改为分专业方向上传</a>
				</c:if>

				<c:if test="${user.adminlevel == 5}">
					<a
						href="topology_revisechangetag.action?department.departmentid=${department.departmentid }">改为分专业方向上传</a>
				</c:if>
			</div>
			<div id="box1">
		...
		<div style="width: 90%; margin-left: 29px; margin-top: 26px;">
			<c:if test="${not empty msg }">
