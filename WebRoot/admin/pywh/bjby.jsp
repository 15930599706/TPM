			...
			<table border="0" cellspacing="0"
				...
				<tr v-for="item in arr">
					...
					<td><span style="padding-left: 5px; padding-right: 10px;">{{
							item.point_arr.length }}</span> <%--<button v-on:click="addPoint(item)">添加</button>
					--%></td>
					<td>
						<div v-for="(point, index) in item.point_arr">
							<textarea rows="1" v-model="item.point_arr[index]"
								style="width: 85%" disabled></textarea>
							<%--<input type="button" name="" id="" value="删除" v-on:click="deletePoint(item, index)" />
						--%>
						</div>
					</td>
				</tr>
			</table>
			<%--<button @click="submitData"style="margin-left:50%;margin-top:10px;">提交</button>
			--%>
			<div>{{ tip }}</div>
		</div>
	</div>
...
<script type="text/javascript">
		var vm = new Vue({
			...
			methods: {
				...
				submitData: function() {
					if(Array.prototype.some.call($("textarea"), function(item) {
						return item.value.trim() == "";
					})) {
						alert("指标点内容不能有空项!");
						return;
					}
					showMask({ info: "正在提交，请稍后……" });
					...
					this.$http.post(url, { // 数据
						...
						if(${user.adminlevel == 1 }){
							window.location.href = "${pageContext.request.contextPath}/curriculumMatrix_tobjjzpage.action?departmentid=${department.departmentid}";
						}
						if(${user.adminlevel != 1 }){
							window.location.href = "${pageContext.request.contextPath}/curriculumMatrix_tozyjzpage.action?tnum=${user.tnum }&collegeid=-1";
						}
