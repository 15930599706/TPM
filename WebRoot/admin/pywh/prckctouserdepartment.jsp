...
<body>
	<div class="right_cont">
		...
		<div id="box2">
			<form
				action="${pageContext.request.contextPath}/practicePlan_kctouserpagedepartment.action?curriculumid=${curriculum.curriculumid }"
		...
		<div id="box2">
			分配课程：${curriculum.curriculumCname } <input type="hidden"
				name="curriculum.curriculumid" value="${curriculum.curriculumid}">
			&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;
		</div>
		<div id="xuanke">
			...
			<%-- <c:if test="${newCurriculum.user.username ne null }"><td style="color: red;">当前老师：${newCurriculum.user.username }</td></c:if> --%>
		</div>
		<div style="width: 90%; margin-left: 24px; margin-top: 10px;">
			...
			<table class="table table-bordered table-striped table-hover">
				...
				<c:forEach items="${pageBean.userlist }" var="nuser">
					<tr align="center">
						...
						<td nowrap="nowrap"><small> <a
								href="${pageContext.request.contextPath}/practicePlan_kctouserbydepart.action?
								curriculum.curriculumid=${curriculum.curriculumid }&teacher.tnum=${nuser.tnum }&
								<c:forEach items="${pageBean.professionallist}" var="professional">
				...
				<form method="post"
					action="${pageContext.request.contextPath}/theoreticalPlan_kctouserpagedepartment.action?curriculum.college.collegeid=${user.college.collegeid}&
					teachDepartment.departmentid=<c:forEach items="${pageBean.departmentlist }" var="department">${department.departmentid}</c:forEach>&usertag=0&
					...
					<tfoot>
						<tr align="center">
							<th nowrap="nowrap" colspan="11"><c:if
									...
									<a
										href="${pageContext.request.contextPath}/practicePlan_kctouserpagedepartment.action?curriculum.college.collegeid=${user.college.collegeid}&
									teachDepartment.departmentid=<c:forEach items="${pageBean.departmentlist }" var="department">${department.departmentid}</c:forEach>&curriculum.curriculumid=${curriculum.curriculumid}&curriculum.curriculumCname=${curriculum.curriculumCname}&usertag=0&
									&teacher.tnum=${id}&teacher.username=${uname}<c:forEach items="${pageBean.professionallist}" var="professional"><c:if test="${professional.professionalname ne null}">&depart=${professional.professionalid }</c:if><c:if test="${professional.professionalname eq null}">&depart=${professional.department.departmentid}</c:if></c:forEach>"><small>首页</small>
									</a>&nbsp;&nbsp;&nbsp;
										<a
										href="${pageContext.request.contextPath}/practicePlan_kctouserpagedepartment.action?currentpage=${currentpage-1 }&curriculum.college.collegeid=${user.college.collegeid}&
									teachDepartment.departmentid=<c:forEach items="${pageBean.departmentlist }" var="department">${department.departmentid}</c:forEach>&curriculum.curriculumid=${curriculum.curriculumid}&curriculum.curriculumCname=${curriculum.curriculumCname}&usertag=0&
								...
								<c:if test="${pageBean.currentpage != pageBean.totalPage }">
									<a
										href="${pageContext.request.contextPath}/practicePlan_kctouserpagedepartment.action?currentpage=${currentpage+1 }&curriculum.college.collegeid=${user.college.collegeid}&
									teachDepartment.departmentid=<c:forEach items="${pageBean.departmentlist }" var="department">${department.departmentid}</c:forEach>
									&curriculum.curriculumid=${curriculum.curriculumid}&curriculum.curriculumCname=${curriculum.curriculumCname}&usertag=0
									&teacher.tnum=${id}&teacher.username=${uname}<c:forEach items="${pageBean.professionallist}" var="professional"><c:if test="${professional.professionalname ne null}">&depart=${professional.professionalid }</c:if><c:if test="${professional.professionalname eq null}">&depart=${professional.department.departmentid}</c:if></c:forEach>">
										<small>下一页</small>
									</a>&nbsp;&nbsp;&nbsp;
								<a
										href="${pageContext.request.contextPath}/practicePlan_kctouserpagedepartment.action?currentpage=${pageBean.totalPage }&curriculum.college.collegeid=${user.college.collegeid}&
									teachDepartment.departmentid=<c:forEach items="${pageBean.departmentlist }" var="department">${department.departmentid}</c:forEach>
									&curriculum.curriculumid=${curriculum.curriculumid}&curriculum.curriculumCname=${curriculum.curriculumCname}&usertag=0
									&teacher.tnum=${id}&teacher.username=${uname}<c:forEach items="${pageBean.professionallist}" var="professional"><c:if test="${professional.professionalname ne null}">&depart=${professional.professionalid }</c:if><c:if test="${professional.professionalname eq null}">&depart=${professional.department.departmentid}</c:if></c:forEach>">
										<small>尾页</small>
	...
	function selectSubmit()
		{
			...
			form.action="${pageContext.request.contextPath}/curriculum_kctouserpagedepartment.action?curriculumid=${curriculum.curriculumid }";
			form.submit();  
		}
