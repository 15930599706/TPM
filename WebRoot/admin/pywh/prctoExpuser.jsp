...
<body>
	<div class="right_cont">
		...
		<div id="box2">
			<form
				action="${pageContext.request.contextPath}/practicePlan_prctoExpuser.action?tnum=${user.tnum }"
	...
	<div style="width: 90%; margin-left: 95px;">
		...
		<table class="table table-bordered table-striped table-hover">
						...
						<form method="post" id="like${xh.count}"
							...
							<select class="example-selectAllNumber" multiple="multiple"
								...
								<c:forEach items="${pageBean.practiceLessonlist[xh.count-1]}"
									var="practiceLesson">
									<label> <c:if
											test="${practiceLesson.experimenter.username ne null}">
											<span style="color: green;"> <c:if
													test="${practiceLesson.professional.professionalname ne null}">
													<option
														value="${practiceLesson.professional.professionalid}">${practiceLesson.department.departmentCname}（${practiceLesson.professional.professionalname}）——${practiceLesson.experimenter.username }</span>
											</option>
										</c:if> <c:if
											test="${practiceLesson.professional.professionalname eq null}">
											<span style="color: green;">
												<option value="${practiceLesson.department.departmentid}">${practiceLesson.department.departmentCname}——${practiceLesson.experimenter.username }
											</span>
											</option>
										</c:if> </c:if> <!-- 已分配 --> <c:if
											test="${practiceLesson.experimenter.username eq null}">
			...
			<form method="post" name="form11"
				action="${pageContext.request.contextPath}/practicePlan_prctoExpuser.action?tnum=${user.tnum }&curriculum.college.collegeid=
	...
	<script language="javascript">
		...
		{	
			...
			/* alert(form); */
			form.action="${pageContext.request.contextPath}/practicePlan_protoexpkctouser.action?experimenter.tnum&experimenter.username&departmenttag&newcurrentpage=${currentpage}";
			form.submit();  
		}
</script>
