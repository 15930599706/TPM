...
<body>
	<div class="right_cont">
		...
		<form
			action="${pageContext.request.contextPath}/practicePlan_touserdepartment.action"
			...
			<div id="box2">
				...
				<td><input size="1" name="curriculum.curriculumCname"
					class="span3" value="${cname}" style="width: 135px" /></td>
				<tr>
	...
	<div style="width: 90%; margin-left: 24px;">
		<table class="table table-bordered table-striped table-hover">
						...
						<form method="post" id="like${xh.count}"
							...
							<select class="example-selectAllNumber" multiple="multiple"
								...
								<c:forEach items="${pageBean.practiceLessonlist[xh.count-1]}"
									...
									<label><c:if
											test="${theoreticalLesson.teacher.username ne null}">
											<span style="color: green;"> <c:if
													test="${theoreticalLesson.professional.professionalname ne null}">
													<option
														value="${theoreticalLesson.professional.professionalid}">${theoreticalLesson.department.departmentCname}（${theoreticalLesson.professional.professionalname}）——${theoreticalLesson.teacher.username}</option></span>

										</c:if> <c:if
											test="${theoreticalLesson.professional.professionalname eq null}">
											<span style="color: green;">
												<option value="${theoreticalLesson.department.departmentid}">${theoreticalLesson.department.departmentCname}——${theoreticalLesson.teacher.username}</option>
											</span>

										</c:if> </c:if> <c:if test="${theoreticalLesson.teacher.username eq null}">
											<span style="color: red;"> <c:if
													test="${theoreticalLesson.professional.professionalname ne null}">
													<option
														value="${theoreticalLesson.professional.professionalid}">${theoreticalLesson.department.departmentCname}（${theoreticalLesson.professional.professionalname}）——未分配！
													</option></span>
			...
			<form method="post" name="form11"
				action="${pageContext.request.contextPath}/practicePlan_touserdepartment.action?teachDepartment.departmentid=${user.department.departmentid}&curriculum.college.collegeid=
				...
				<tfoot>
					<tr align="center">
						<th nowrap="nowrap" colspan="11"><c:if
								...
								<a
									href="${pageContext.request.contextPath}/practicePlan_touserdepartment.action?teachDepartment.departmentid=${user.department.departmentid}&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&usertag=${newusertag}"><small>首页</small>
								</a>&nbsp;&nbsp;&nbsp;
										<a
									href="${pageContext.request.contextPath}/practicePlan_touserdepartment.action?teachDepartment.departmentid=${user.department.departmentid}&currentpage=${currentpage-1 }&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&usertag=${newusertag}"><small>上一页</small>
								</a>
							</c:if>&nbsp;&nbsp;&nbsp; <small>共${pageBean.totalCount
									}条记录,共计${pageBean.totalPage }页,当前第${pageBean.currentpage }页</small>&nbsp;&nbsp;&nbsp;
							<c:if test="${pageBean.currentpage != pageBean.totalPage }">
								<a
									href="${pageContext.request.contextPath}/practicePlan_touserdepartment.action?currentpage=${currentpage+1 }&teachDepartment.departmentid=${user.department.departmentid}&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&usertag=${newusertag}"><small>下一页</small>
								</a>&nbsp;&nbsp;&nbsp;
										
										<a
									href="${pageContext.request.contextPath}/practicePlan_touserdepartment.action?currentpage=${pageBean.totalPage }&teachDepartment.departmentid=${user.department.departmentid}&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&usertag=${newusertag}"><small>尾页</small>
								</a>
							</c:if> 第</small><input name="currentpage"
							onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
							class="span1" /><small>页 <input type="submit" value="跳转">&nbsp;&nbsp;&nbsp;</th>
					</tr>
	...
	<script language="javascript">
		function selectonSubmit(value) {
			...
			form.action = "${pageContext.request.contextPath}/practicePlan_kctouserpagedepartment.action?teacher.tnum=&${id}&teacher.username=${uname}";
			form.submit();
		}
	</script>
	<script type="text/javascript">
