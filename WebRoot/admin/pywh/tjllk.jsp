...
<script language="javascript">
		...
		{
			var form=document.getElementById("tjllk");
			form.action="${pageContext.request.contextPath}/theoreticalPlan_tjllkResult.action?collegeid=${college.collegeid}&departmentid=${department.departmentid}";
			form.submit();  			
		}
	</script>
...
<body>
	<div class="right_cont">
		...
		<form method="post" id="tjllk" onsubmit="selectSubmit()">
			...
			<div id="box2">
				...
				<td><select size="1" name="curriculum.courseLei"
					onChange="selectSubmit()" style="width: 90px;">
						<option value="">【全部】</option>
						<option value="理论课"
							<c:if test="${leibie == '理论课' }">selected="selected"</c:if>>理论课</option>
						<option value="实验课"
							<c:if test="${leibie == '实验课' }">selected="selected"</c:if>>实验课</option>
						<option value="体育课"
							<c:if test="${leibie == '体育课' }">selected="selected"</c:if>>体育课</option>
				</select></td>
			...
			<div id="box2">
				...
				<td><input size="1" name="curriculum.curriculumid"
					class="span3" value="${daima }" style="width: 110px" /></td>
				<td width="40%" align="right" nowrap="nowrap">课程名称：</td>
				<td><input size="1" name="curriculum.curriculumCname"
					class="span3" value="${mingcheng }" style="width: 160px" /></td>
				<tr>
				...
				<table class="table table-bordered table-striped table-hover">
					<thead>
						<tr align="center">
							...
							<th width="80" nowrap="nowrap"><small>开课学期</small></th>
							<th width="80" nowrap="nowrap"><small>是否学位课</small></th>
							<th width="80" nowrap="nowrap"><small>是否教改</small></th>
							<th width="80" nowrap="nowrap"><small>是否必选</small></th>
					...
					<tbody>
						...
						<c:forEach items="${curriculumList }" var="curriculum"
							...
							<tr align="center">
								...
								<td nowrap="nowrap"><select class="isjiaogai"
									style="width: 50px;">
										<option value="是">是</option>
										<option value="否" selected>否</option>
								</select></td>
								...
								<td nowrap="nowrap"><c:if
										test="${curriculum.natureOfCourse.natureOfCoursename ne '专业选修课' }">
										<select class="isbixuan" style="width: 50px;" disabled>
											<option value=""></option>
										</select>
									</c:if> <c:if
										test="${curriculum.natureOfCourse.natureOfCoursename eq '专业选修课' }">
										<select class="isbixuan" style="width: 50px;">
											<option value="是">是</option>
											<option value="否" selected>否</option>
										</select>
									</c:if></td>
								...
								<c:if test="${professionalListCount > 0}">
									<td nowrap="nowrap"><select size="1"
										...
										multiple="multiple"
										<c:if test="${curriculum.curriculumpingtai eq '公共教育平台' || curriculum.natureOfCourse.natureOfCoursename eq '专业必选课'}">disabled</c:if>>
	...
	<script type="text/javascript">  
        ...
        function a() {
	        ...
	        var result = getChildValues($parents, {	     
	        	...
	        	isjiaogai: { selector: ".isjiaogai", prop: "value" },	
	        	isbixuan: { selector: ".isbixuan", prop: "value" },        	
	        	zhuanyefangxiang: { 
	        ...
	        var url = "${pageContext.request.contextPath}/theoreticalPlan_llkAddResult.action?departmentid=${department.departmentid}";//?tnum=${user.tnum}&data="+encodeURI(encodeURI(JSON.stringify(result)));
			$.post(url, {
		//		tnum: ${user.tnum},
			...
			//	}else{
					hiddenMask();
					alert("添加成功，点击确定查看");
					window.location.href = "${pageContext.request.contextPath}/theoreticalPlan_tollkPage.action?tnum=${user.tnum}&collegeid=${college.collegeid}&departmentid=${department.departmentid}";
			//	}
			});
        }
