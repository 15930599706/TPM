...
<div class="right_cont">
	...
	<div id="box2">
		<form
			action="${pageContext.request.contextPath}/theoreticalPlan_toExpusercollege.action?tnum=${user.tnum }"
			...
			<td><input size="1" name="curriculum.curriculumid" class="span3"
	...
	<%--  <div style="width:1000px;">
		     <div id="box0">
		        <button  class="btn btn-info  "><a href="${pageContext.request.contextPath}/theoreticalPlan_A_key_distribution.action?curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=0" >一键分配</a></button>				
		     </div>
		  </div>
		  <div id="box00">
...
<div style="width: 90%; margin-left: 95px;">
	...
	<table class="table table-bordered table-striped table-hover">
							...
							<c:forEach items="${pageBean.theoreticalLessonlist[xh.count-1]}"
								...
								<label> <c:if
										...
										<c:if
											test="${theoreticalLesson.professional.professionalname eq null}">
											<span style="color: green;">
												<option value="${theoreticalLesson.department.departmentid}">${theoreticalLesson.department.departmentCname}——${theoreticalLesson.cteachDepartment.departmentCname }
											</span>
											</option>
										</c:if>
										...
										<c:if
											test="${theoreticalLesson.professional.professionalname eq null}">
											<span style="color: green;">
												<option value="${theoreticalLesson.department.departmentid}">${theoreticalLesson.department.departmentCname}——${theoreticalLesson.experiment.experimentname }
											</span>
											</option>
										</c:if>
									</c:if> <!-- 如果实验员不空时 --> <c:if
										...
										<c:if
											test="${theoreticalLesson.professional.professionalname eq null}">
											<span style="color: green;">
												<option value="${theoreticalLesson.department.departmentid}">${theoreticalLesson.department.departmentCname}——${theoreticalLesson.experimenter.username }(实验员)
											</span>
											</option>
										</c:if>
				...
				<td nowrap="nowrap"><small> <c:forEach
							...
							<c:if test="${theoreticalLesson.experimenter.username eq null}">
								<a href="javascript:void(0)"
									onclick="selectonSubmit('${xh.count}')">分配任课实验员</a>
							</c:if>
							<c:if test="${theoreticalLesson.experimenter.username ne null}">
								<a href="javascript:void(0)"
									onclick="selectonSubmit('${xh.count}')">修改任课实验员</a>
							</c:if>
						</c:forEach>
				</small></td>
		...
		<form method="post" name="form11"
			action="${pageContext.request.contextPath}/theoreticalPlan_toExpusercollege.action?tnum=${user.tnum }&curriculum.college.collegeid=
					...
					<th nowrap="nowrap" colspan="11"><c:if
							...
							<a
								href="${pageContext.request.contextPath}/theoreticalPlan_toExpusercollege.action?tnum=${user.tnum }&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=0"><small>首页</small></a>&nbsp;&nbsp;&nbsp;
										<a
								href="${pageContext.request.contextPath}/theoreticalPlan_toExpusercollege.action?tnum=${user.tnum }&currentpage=${currentpage-1 }&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=${newdepartmenttag}"><small>上一页</small></a>
						...
						<c:if test="${pageBean.currentpage != pageBean.totalPage }">
							<a
								href="${pageContext.request.contextPath}/theoreticalPlan_toExpusercollege.action?tnum=${user.tnum }&currentpage=${currentpage+1 }&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=${newdepartmenttag}"><small>下一页</small></a>&nbsp;&nbsp;&nbsp;
										<a
								href="${pageContext.request.contextPath}/theoreticalPlan_toExpusercollege.action?tnum=${user.tnum }&currentpage=${pageBean.totalPage }&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=${newdepartmenttag}"><small>尾页</small></a>
						</c:if> <small> 第</small><input type="number" name="currentpage"
						onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"
						class="span1" /><small>页 </small> <input type="submit" value="跳转">&nbsp;&nbsp;&nbsp;
					</th>
				</tr>
...
<script language="javascript">
		...
		{	
			...
			/* alert(form); */
			form.action="${pageContext.request.contextPath}/theoreticalPlan_expkctousercollege.action?experimenter.tnum&experimenter.username&departmenttag&newcurrentpage=${currentpage}";
			form.submit();  
		}
</script>
