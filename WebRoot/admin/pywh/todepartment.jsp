...
<body>
	<div class="right_cont">
		...
		<div style="width: 1000px;">
			<div id="box0">
				<button class="btn btn-info  ">
					<a
						href="${pageContext.request.contextPath}/theoreticalPlan_A_key_distribution.action?curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=0">一键分配</a>
				</button>
			</div>
		</div>
		<div id="box00">
		...
		<%-- <div style="width:1000px;">
		    <div id="box0">
		        <button  class="btn btn-info  "><a href="${pageContext.request.contextPath}/theoreticalPlan_exportTheoryclassExcel.action?curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=0" >导出课表</a></button>		
			</div>
		  </div> --%>
		...
		<div style="width: 1000px;">
			<div id="box0">
				<button class="btn btn-info  ">
					<a
						href="${pageContext.request.contextPath}/theoreticalPlan_exportTheoryclassExcel.action?curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=0">导出课表</a>
				</button>
			</div>
		</div>
	...
	<div style="width: 90%; margin-left: 95px;">
		...
		<table class="table table-bordered table-striped table-hover">
				...
				<tr align="center">
						...
						<form method="post" id="like${xh.count}"
							...
							<select class="example-selectAllNumber" multiple="multiple"
								...
								<c:forEach items="${pageBean.theoreticalLessonlist[xh.count-1]}"
									...
									<label> <c:if
											test="${theoreticalLesson.teachDepartment.departmentCname ne null or theoreticalLesson.teacher.username ne null}">
											<span style="color: green;"> <c:if
													test="${theoreticalLesson.teachDepartment.departmentCname ne null and theoreticalLesson.teacher.username eq null}">
													<c:if
														test="${theoreticalLesson.professional.professionalname ne null}">
														<option
															value="${theoreticalLesson.professional.professionalid}">${theoreticalLesson.department.departmentCname}（${theoreticalLesson.professional.professionalname}）——${theoreticalLesson.teachDepartment.departmentCname }</span>
											</option>
										</c:if> <c:if
											test="${theoreticalLesson.professional.professionalname eq null}">
											<span style="color: green;">
												<option value="${theoreticalLesson.department.departmentid}">${theoreticalLesson.department.departmentCname}——${theoreticalLesson.teachDepartment.departmentCname}
											</span>
											</option>
										</c:if> </c:if> <c:if
											test="${theoreticalLesson.teacher.username ne null and theoreticalLesson.teachDepartment.departmentCname eq null}">
											<c:if
												test="${theoreticalLesson.professional.professionalname ne null}">
												<option
													value="${theoreticalLesson.professional.professionalid}">${theoreticalLesson.department.departmentCname}（${theoreticalLesson.professional.professionalname}）——${theoreticalLesson.teacher.username}</span></option>
											</c:if>
											<c:if
												test="${theoreticalLesson.professional.professionalname eq null}">
												<span style="color: green;">
													<option
														value="${theoreticalLesson.department.departmentid}">${theoreticalLesson.department.departmentCname}——${theoreticalLesson.teacher.username}
												</span>
												</option>
											</c:if>
										</c:if> <c:if
											test="${theoreticalLesson.teacher.username ne null and theoreticalLesson.teachDepartment.departmentCname ne null}">
											<c:if
												test="${theoreticalLesson.professional.professionalname ne null}">
												<option
													value="${theoreticalLesson.professional.professionalid}">${theoreticalLesson.department.departmentCname}（${theoreticalLesson.professional.professionalname}）——${theoreticalLesson.teacher.username}</span></option>
											...
											test="${theoreticalLesson.teachDepartment.departmentCname eq null and theoreticalLesson.teacher.username eq null}">
					...
					<td nowrap="nowrap"><small> <c:forEach
								...
								<c:if
									...
									<%-- (<c:if test="${theoreticalLesson.teachDepartment.departmentCname ne null}"><span style="color: green;">专业</span></c:if><c:if test="${theoreticalLesson.teachDepartment.departmentCname eq null and theoreticalLesson.teacher.username ne null}"><span style="color: green;">教师</span></c:if>)</span> --%>
								</c:if>
					...
					<td nowrap="nowrap"><small> <c:forEach
								...
								var="theoreticalLesson" end="0">
								<c:if
									test="${theoreticalLesson.teachDepartment.departmentCname eq null}">
									<a href="javascript:void(0)"
										onclick="selectonSubmit('${xh.count}')">分配开课专业</a>
								</c:if>
								<c:if
									test="${theoreticalLesson.teachDepartment.departmentCname ne null}">
									<a href="javascript:void(0)"
										onclick="selectonSubmit('${xh.count}')">修改开课专业</a>
								</c:if>
							</c:forEach> <%-- <c:if test="${curriculum.department.departmentCname ne null}"><a href="${pageContext.request.contextPath}/curriculum_gogivedepartment.action?curriculumid=${curriculum.curriculumid }&college.collegeid=${user.college.collegeid}">修改开课专业</a></c:if> --%>
					</small></td>
			...
			<form method="post" name="form11"
				action="${pageContext.request.contextPath}/theoreticalPlan_todepartment.action?curriculum.college.collegeid=
				...
				<tfoot>
					<tr align="center">
						<th nowrap="nowrap" colspan="11"><c:if
								...
								<a
									href="${pageContext.request.contextPath}/theoreticalPlan_todepartment.action?curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=0"><small>首页</small></a>&nbsp;&nbsp;&nbsp;
										<a
									href="${pageContext.request.contextPath}/theoreticalPlan_todepartment.action?currentpage=${currentpage-1 }&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&departmenttag=${newdepartmenttag}"><small>上一页</small></a>
	...
	<script language="javascript">
		...
		{	
			...
			/* alert(form); */
			form.action = "${pageContext.request.contextPath}/theoreticalPlan_gogivedepartment.action?departmenttag&newcurrentpage=${currentpage}";
			form.submit();  
		}
		</script>
