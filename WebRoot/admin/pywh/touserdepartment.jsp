...
<body>
	<div class="right_cont">
		...
		<form
			action="${pageContext.request.contextPath}/theoreticalPlan_touserdepartment.action"
	...
	<div style="width: 90%; margin-left: 24px; margin-top: 70px;">
		<table class="table table-bordered table-striped table-hover">
								...
								<c:forEach items="${pageBean.theoreticalLessonlist[xh.count-1]}"
									...
									<label> <c:if
											...
											<span style="color: green;"> <c:if
													...
													<option
														value="${theoreticalLesson.professional.professionalid}">${theoreticalLesson.department.departmentCname}（${theoreticalLesson.professional.professionalname}）——${theoreticalLesson.teacher.username
															}</span>
											</option>
										</c:if> <c:if
											test="${theoreticalLesson.professional.professionalname eq null}">
											<span style="color: green;">
												<option value="${theoreticalLesson.department.departmentid}">${theoreticalLesson.department.departmentCname}——${theoreticalLesson.teacher.username
														}
											</span>
											</option>
			...
			<form method="post" name="form11"
				action="${pageContext.request.contextPath}/theoreticalPlan_touserdepartment.action?teachDepartment.departmentid=${user.department.departmentid}&curriculum.college.collegeid=
				...
				<tfoot>
					<tr align="center">
						<th nowrap="nowrap" colspan="11"><c:if
								...
								<a
									href="${pageContext.request.contextPath}/theoreticalPlan_touserdepartment.action?teachDepartment.departmentid=${user.department.departmentid}&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&usertag=${newusertag}"><small>首页</small>
								</a>&nbsp;&nbsp;&nbsp;
										<a
									href="${pageContext.request.contextPath}/theoreticalPlan_touserdepartment.action?teachDepartment.departmentid=${user.department.departmentid}&currentpage=${currentpage-1 }&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&usertag=${newusertag}"><small>上一页</small>
								</a>
							</c:if>&nbsp;&nbsp;&nbsp; <small>共${pageBean.totalCount
									}条记录,共计${pageBean.totalPage }页,当前第${pageBean.currentpage }页</small>&nbsp;&nbsp;&nbsp;
							<c:if test="${pageBean.currentpage != pageBean.totalPage }">
								<a
									href="${pageContext.request.contextPath}/theoreticalPlan_touserdepartment.action?currentpage=${currentpage+1 }&teachDepartment.departmentid=${user.department.departmentid}&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&usertag=${newusertag}"><small>下一页</small>
								</a>&nbsp;&nbsp;&nbsp;
										
										<a
									href="${pageContext.request.contextPath}/theoreticalPlan_touserdepartment.action?currentpage=${pageBean.totalPage }&teachDepartment.departmentid=${user.department.departmentid}&curriculum.college.collegeid=${user.college.collegeid}&curriculum.curriculumid=${theoreticalLesson.curriculum.curriculumid}&curriculum.curriculumCname=${theoreticalLesson.curriculum.curriculumCname}&usertag=${newusertag}"><small>尾页</small>
								</a>
							</c:if> <small> 第</small><input type="number" name="currentpage"
							onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"
							class="span1" /><small>页 </small><input type="submit" value="跳转">&nbsp;&nbsp;&nbsp;</th>
					</tr>
	...
	<script language="javascript">
		function selectonSubmit(value) {
			...
			form.action = "${pageContext.request.contextPath}/theoreticalPlan_kctouserpagedepartment.action?teacher.tnum=&${id}&teacher.username=${uname}";
			form.submit();
		}
	</script>
	<script type="text/javascript">
