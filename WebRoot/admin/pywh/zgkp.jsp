...
<body>
	<div class="right_cont">
		...
		<div id="box2">
			<c:if test="${user.adminlevel == 5 }">
				<form
					action="${pageContext.request.contextPath}/topology_tozgkpPage.action?tnum=${user.tnum }"
		...
		<c:if test="${pageBean.totalCount != 0 }">
			<div style="width: 70%; margin-left: 24px;">
				<form
					action="${pageContext.request.contextPath}/topology_tozgkpPage.action?tnum=${user.tnum }&collegeid=${collegeid}"
					...
					<table class="table table-bordered table-striped table-hover">
						...
						<c:forEach items="${pageBean.departmentlist }" var="department"
							...
							<tr align="center" class="even">
								...
								<td nowrap="nowrap"><a
									href="${pageContext.request.contextPath}/topology_toViewTopologyPage.action?department.departmentid=${department.departmentid }"><small>查看</small></a>
								</td>
								<c:if test="${user.adminlevel == 5 }">
									<td nowrap="nowrap"><a
										href="${pageContext.request.contextPath}/topology_revisetoViewTopologyPage.action?department.departmentid=${department.departmentid }"><small>修改</small></a>
									</td>
								</c:if>
							</tr>
						</c:forEach>
							...
							<tfoot>
								<tr align="center">
									<th nowrap="nowrap" colspan="9"><c:if
											...
											<a
												href="${pageContext.request.contextPath}/topology_tozgkpPage.action?tnum=${user.tnum }&collegeid=${collegeid}"><small>首页</small></a>&nbsp;&nbsp;&nbsp;
									<a
												href="${pageContext.request.contextPath}/topology_tozgkpPage.action?currentpage=${currentpage-1 }&tnum=${user.tnum }&collegeid=${collegeid}"><small>上一页</small></a>
										</c:if>&nbsp;&nbsp;&nbsp; <small>共${pageBean.totalCount }条记录,共计${pageBean.totalPage }页,当前第${pageBean.currentpage }页</small>&nbsp;&nbsp;&nbsp;
										<c:if test="${pageBean.currentpage != pageBean.totalPage }">
											<a
												href="${pageContext.request.contextPath}/topology_tozgkpPage.action?currentpage=${currentpage+1 }&tnum=${user.tnum }&collegeid=${collegeid}"><small>下一页</small></a>&nbsp;&nbsp;&nbsp;
									<a
												href="${pageContext.request.contextPath}/topology_tozgkpPage.action?currentpage=${pageBean.totalPage }&tnum=${user.tnum }&collegeid=${collegeid}"><small>尾页</small></a>
	...
	<script language="javascript">
		function selectSubmit()
		{
			...
			form.action="${pageContext.request.contextPath}/topology_tozgkpPage.action?tnum=${user.tnum }";
			form.submit();  
		}
</script>
</body>
