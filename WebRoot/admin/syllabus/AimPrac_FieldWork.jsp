	...
	<div class="right_cont">
		<div id="container">
			...
			<div id="box9">
				...
				<c:if test="${newtpracticeLesson.curriculum.courseLei eq '实践课'}">
					<a
			...
			<div id="box1">
				...
				<p v-for="(item, index) in teachingGoals">
					...
					<span>{{ index+1 }}、{{ item.teachObjContent_FieldWork }}</span>
				</p>
			</div>
					...
					<tr v-for="(item, index) in relations">
						...
						<td style="width: 100px; text-align: left;">
							...
							<p v-for="(goal, goal_index) in teachingGoals">
								<input type="checkbox"
									v-model="item.goals[goal.teachObj_FieldWorkid]" /><span>教学目标{{
									goal_index + 1 }}</span>
							</p>
						</td>
					</tr>
			...
			var vm = new Vue({
				...
				methods: {
					addGoal: function() {
						...
						var url = "${pageContext.request.contextPath}/teachObj_addTeachObjFieldWork.action?syllabusId=${syllabusId}";
						that.$http.post(url, {
							...
							if(res.body.error) {
								...
								that.teachingGoals.push({
									teachObj_FieldWorkid: parseInt(res.body.message),
									teachObjContent_FieldWork: that.tempGoalContent
								});
								alert("添加教学目标成功！");
					...
					removeGoal: function(index) {
						var url = "${pageContext.request.contextPath}/teachObj_delTeachObjFieldWork.action?";
						var that = this;
						this.$http.post(url, {
							data: that.teachingGoals[index].teachObj_FieldWorkid
						}, g_common_config).then(function(res) {
					...
					refreshRelations: function() {
						...
						that.relations.forEach(function(relation) {
							...
							that.teachingGoals.forEach(function(goal) {
								goals[goal.teachObj_FieldWorkid] = relation.goals[goal.teachObj_FieldWorkid] == true;
							});
					...
					var emptyEle = [];
						...
						var url = "${pageContext.request.contextPath}/abilityAndTeachObj_addAbilityAndTeachObjFieldWork.action?syllabusId=${syllabusId}";
						this.$http.post(url, {
							data: JSON.stringify(this.c_result)
						}, g_common_config).then(function(res) {
							alert("保存成功，请填写其他内容！");
							window.location.href = "${pageContext.request.contextPath}/practicePlan_toHavePracLesFieldWorkPage.action?tnum=${user.tnum}&syllabusId=${syllabusId}&practiceLessonid=${newpracticeLesson.practiceLessonid }&curriculum.curriculumid=${newpracticeLesson.curriculum.curriculumid }&department.departmentid=${newpracticeLesson.department.departmentid}";
						});
					}
				},
				created: function() {
