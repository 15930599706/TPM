		...
		<div id="container">
			...
			<div id="box1">
				...
				<p v-for="(item, index) in teachingGoals">
					...
					<span>{{ index+1 }}、{{ item.teachObjContent }}</span>
				</p>
			</div>
			...
			<div id="box2">
					...
					<tr v-for="(item, index) in relations">
						...
						<td style="width: 100px; text-align: left;">
							...
							<p v-for="(goal, goal_index) in teachingGoals">
								<input type="checkbox" v-model="item.goals[goal.teachObjid]" /><span>教学目标{{
									goal_index + 1 }}</span>
							</p>
						</td>
					</tr>
				...
				<p>
					<c:if test="${newtheoreticalLesson.curriculum.courseLei eq '理论课'}">
						<button
							v-on:click="save(${newtheoreticalLesson.theoreticalLessonid})">提交</button>
					</c:if>
					<c:if test="${newtheoreticalLesson.curriculum.courseLei eq '实践课'}">
						<button
							v-on:click="save(${newtheoreticalLesson.practiceLessonid})">提交</button>
					</c:if>
				</p>
			</div>
		</div>
			...
			var vm = new Vue({
				...
				methods: {
					addGoal: function() {
						...
						var url = "${pageContext.request.contextPath}/teachObj_addTeachObj.action?syllabusId=${syllabusId}";
						that.$http.post(url, {
							...
							if(res.body.error) {
								...
								that.teachingGoals.push({
									teachObjid: parseInt(res.body.message),
									teachObjContent: that.tempGoalContent
								});
								alert("添加教学目标成功！");
					...
					removeGoal: function(index) {
						var url = "${pageContext.request.contextPath}/teachObj_delTeachObj.action?";
						var that = this;
						this.$http.post(url, {
							data: that.teachingGoals[index].teachObjid
						}, g_common_config).then(function(res) {
					...
					refreshRelations: function() {
						...
						that.relations.forEach(function(relation) {
							...
							that.teachingGoals.forEach(function(goal) {
								goals[goal.teachObjid] = relation.goals[goal.teachObjid] == true;
							});
					...
					save: function(id) {
						...
						var url = "${pageContext.request.contextPath}/abilityAndTeachObj_addAbilityAndTeachObj.action?syllabusId=${syllabusId}";
						this.$http.post(url, {
							data: JSON.stringify(this.c_result)
						}, g_common_config).then(function(res) {
							alert("保存成功，请填写其他大纲内容！");
							
				 			if('${newtheoreticalLesson.curriculum.courseLei}' == '理论课')
