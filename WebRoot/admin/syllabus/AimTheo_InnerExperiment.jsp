		...
		<div id="container">
			...
			<div id="box1">
				...
				<p v-for="(item, index) in teachingGoals">
					...
					<span>{{ index+1 }}、{{
						item.teachObjContent_TheoInnerExperiment }}</span>
				</p>
			</div>
					...
					<tr v-for="(item, index) in relations">
						...
						<td style="width: 100px; text-align: left;">
							...
							<p v-for="(goal, goal_index) in teachingGoals">
								<input type="checkbox"
									v-model="item.goals[goal.teachObj_TheoInnerExperimentid]" /><span>教学目标{{
									goal_index + 1 }}</span>
							</p>
						</td>
					</tr>
			...
			var vm = new Vue({
				...
				methods: {
					addGoal: function() {
						...
						var url = "${pageContext.request.contextPath}/teachObj_addTeachObjTheoInnerExperiment.action?syllabusId=${syllabusId}";
						that.$http.post(url, {
							...
							if(res.body.error) {
								...
								that.teachingGoals.push({
									teachObj_TheoInnerExperimentid: parseInt(res.body.message),
									teachObjContent_TheoInnerExperiment: that.tempGoalContent
								});
								alert("添加教学目标成功！");
					...
					removeGoal: function(index) {
						var url = "${pageContext.request.contextPath}/teachObj_delTeachObjTheoInnerExperiment.action?";
						var that = this;
						this.$http.post(url, {
							data: that.teachingGoals[index].teachObj_TheoInnerExperimentid
						}, g_common_config).then(function(res) {
					...
					refreshRelations: function() {
						...
						that.relations.forEach(function(relation) {
							...
							that.teachingGoals.forEach(function(goal) {
								goals[goal.teachObj_TheoInnerExperimentid] = relation.goals[goal.teachObj_TheoInnerExperimentid] == true;
							});
					...
					save: function() {
						...
						var url = "${pageContext.request.contextPath}/abilityAndTeachObj_addAbilityAndTeachObjTheoInnerExperiment.action?syllabusId=${syllabusId}";
						this.$http.post(url, {
