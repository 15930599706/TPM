...
<body>
	<div class="right_cont">
		<div class="title_right">
			<strong>实践课大纲录入——<font style="color: #0000c6;">课程设计（学年论文）</font></strong>
		</div>
		<div class="link">
			<a
		...
		<div class="link1">
			<a
				...
				style="color: #0000c6;">课程设计（学年论文）大纲</font></a>
		</div>
		<div class="link2">
			<a
		...
		<div style="padding-left: 24px; padding-top: 30px;">
					...
					<c:forEach items="${haveSyllabusList}" var="haveSyllabus"
						...
						<tr align="center">
							...
							<td><select size="1" id="load${xh.count}"
								style="width: 10em;" onchange="LoadFlager1('${xh.count}')">
									<c:forEach items="${list}" var="keyword" varStatus="id">
										<option value="${keyword}"
											<c:if test="${keyword eq haveSyllabus.workload1}">
													selected="selected"
										</c:if>>${keyword}</option>
									</c:forEach>
			...
			<div style="border-bottom: #CCCCCC 1 solid;">
				<h4 style="color: red;">
					未编写 “课程设计（学年论文）”大纲：<font style="font-size: 14px;">（注意：复制功能将不复制“教学目标及对应关系”，采用此功能后请再自行填写该项！）</font>
				</h4>
	...
	//数据库传是否
	function LoadFlager1(value){
		var name = "do"+value;
		var form=document.getElementById(name);
		var id="load"+value;
		var value = document.getElementById(id).value;
		if(value=='是'){
			form.action="${pageContext.request.contextPath}/practicePlan_LoadFlagUp1.action?isLoad=1&user.tnum=${user.tnum}";
		}else{
			form.action="${pageContext.request.contextPath}/practicePlan_LoadFlagUp1.action?isLoad=0&user.tnum=${user.tnum}";
		}
