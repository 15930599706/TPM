...
<body>
	<div class="right_cont">
		...
		<div style="padding-left: 24px; padding-top: 30px;">
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<c:forEach items="${haveSyllabusList}" var="haveSyllabus"
						...
						<tr align="center">
							...
							<td nowrap="nowrap"><a
								href="${pageContext.request.contextPath}/ExportWork_exportFieldwork.action?syllabusid=${syllabusId[xh.count-1]}&fieldid=${haveSyllabus.practiceLessonid}"><small>下载</small>
							</a></td>
							<%
							...
							<td><select size="1" id="load${xh.count}"
								style="width: 10em;" onchange="LoadFlager('${xh.count}')">
									<c:forEach items="${list}" var="keyword" varStatus="id">
										<option value="${keyword}"
											<c:if test="${keyword eq haveSyllabus.workload}">selected="selected"</c:if>>${keyword}</option>
									</c:forEach>
							</select></td>
			...
			<div style="border-bottom: #CCCCCC 1 solid;">
				<h4 style="color: red;">
					未编写 “实习” 大纲：<font style="font-size: 14px;">（注意：复制功能将不复制“教学目标及对应关系”，采用此功能后请再自行填写该项！）</font>
				</h4>
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/practicePlan_toPracLesFieldWorkPage.action";
		form.submit();  
	}
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/practicePlan_toHavePracLesFieldWorkPage.action";
		form.submit();  
	}
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/practicePlan_deletePracSyllabusFieldWork.action";
		form.submit();  
	}
	//数据库传是否
	function LoadFlager(value){
		var name = "do"+value;
		var form=document.getElementById(name);
		var id="load"+value;
		var value = document.getElementById(id).value;
		if(value=='是'){
			form.action="${pageContext.request.contextPath}/practicePlan_LoadFlagUp.action?isLoad=1&user.tnum=${user.tnum}";
		}else{
			form.action="${pageContext.request.contextPath}/practicePlan_LoadFlagUp.action?isLoad=0&user.tnum=${user.tnum}";
		}
...
$(document).ready(function() {
	...
	$(".copySelect").on("change", function() {
		...
		form.action="${pageContext.request.contextPath}/practicePlan_toPracLesFieldWorkPageCopy.action?syllabusId_Copy=" + this.value;
		form.submit(); 
	});
</script>
</body>
