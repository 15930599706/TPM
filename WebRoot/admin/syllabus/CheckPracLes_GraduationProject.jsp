...
<body>
	<div class="right_cont">
		<div class="title_right">
			<strong>实践课大纲录入——<font style="color: #0000c6;">毕业设计</font></strong>
		</div>
		<div class="link">
			<a
		...
		<div class="link3">
			<a
				...
				style="color: #0000c6;">毕业设计大纲</font></a>
		</div>
		<div class="link4">
			<a
		...
		<div style="padding-left: 24px; padding-top: 30px;">
			<div style="border-bottom: #CCCCCC 1 solid;">
				<h4 style="color: green;">已编写 “毕业设计” 大纲：</h4>
			</div>
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<thead>
						<tr align="center">
							...
							<th width="80" nowrap="nowrap"><small>可供下载</small></th>
						</tr>
					</thead>
					...
					<c:forEach items="${haveSyllabusList}" var="haveSyllabus"
						...
						<tr align="center">
							...
							<td nowrap="nowrap"><a
								href="${pageContext.request.contextPath}/ExportWork_exportGra.action?syllabusid=${syllabusId[xh.count-1]}&pracid=${haveSyllabus.practiceLessonid}"><small>下载</small></a>
							</td>
							...
							<td><select size="1" id="load${xh.count}"
								style="width: 10em;" onchange="LoadFlager3('${xh.count}')">
									<c:forEach items="${list}" var="keyword" varStatus="id">
										<option value="${keyword}"
											<c:if test="${keyword eq haveSyllabus.workload3}">
													selected="selected"
										</c:if>>${keyword}</option>
									</c:forEach>
			...
			<div style="border-bottom: #CCCCCC 1 solid;">
				<h4 style="color: red;">
					未编写 “毕业设计” 大纲：<font style="font-size: 14px;">（注意：复制功能将不复制“教学目标及对应关系”，采用此功能后请再自行填写该项！）</font>
				</h4>
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/practicePlan_toPracLesGraduationProjectPage.action";
		form.submit();  
	}
	
	//数据库传是否
	function LoadFlager3(value){
		var name = "do"+value;
		var form=document.getElementById(name);
		var id="load"+value;
		var value = document.getElementById(id).value;
		if(value=='是'){
			form.action="${pageContext.request.contextPath}/practicePlan_LoadFlagUp3.action?isLoad=1&user.tnum=${user.tnum}";
		}else{
			form.action="${pageContext.request.contextPath}/practicePlan_LoadFlagUp3.action?isLoad=0&user.tnum=${user.tnum}";
		}
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/practicePlan_toHavePracLesGraduationProjectPage.action";
		form.submit();  
	}
	...
	form.action="${pageContext.request.contextPath}/practicePlan_deletePracSyllabusGraduationProject.action";
	form.submit();  
	}
	</script>
	<script type="text/javascript">
...
$(document).ready(function() {
	...
	$(".copySelect").on("change", function() {
		...
		form.action="${pageContext.request.contextPath}/practicePlan_toPracLesGraduationProjectPageCopy.action?syllabusId_Copy=" + this.value;
		form.submit(); 
	});
</script>
</body>
