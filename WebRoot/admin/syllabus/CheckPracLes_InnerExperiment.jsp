...
<body>
	<div class="right_cont">
		<div class="title_right">
			<strong>实践课大纲录入——<font style="color: #0000c6;">实验</font></strong>
		</div>
		<div class="link">
			<a
		...
		<div class="link2">
			<a
				...
				style="color: #0000c6;">编写实验大纲</font></a>
		</div>
		<div class="link3">
			<a
		...
		<div style="padding-left: 24px; padding-top: 30px;">
			<div style="border-bottom: #CCCCCC 1 solid;">
				<h4 style="color: green;">已编写 “实验” 大纲：</h4>
			</div>
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<c:forEach items="${haveSyllabusList}" var="haveSyllabus"
						...
						<tr align="center">
							...
							<td nowrap="nowrap"><a
								href="${pageContext.request.contextPath}/ExportWork_exportBaseExperiment.action?syllabusid=${syllabusId[xh.count-1]}&expeid=${haveSyllabus.practiceLessonid}"><small>下载</small></a>
							</td>
							<%
							...
							<td><select size="1" id="load${xh.count}"
								style="width: 10em;" onchange="LoadFlager2('${xh.count}')">
									<c:forEach items="${list}" var="keyword" varStatus="id">
										<option value="${keyword}"
											<c:if test="${keyword eq haveSyllabus.workload2}">
													selected="selected"
										</c:if>>${keyword}</option>
									</c:forEach>
			...
			<div style="border-bottom: #CCCCCC 1 solid;">
				<h4 style="color: red;">
					未编写 “实验” 大纲：<font style="font-size: 14px;">（注意：复制功能将不复制“教学目标及对应关系”，采用此功能后请再自行填写该项！）</font>
				</h4>
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/practicePlan_toPracLesInnerExperimentPage.action";
		form.submit();  
	}
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/practicePlan_toHavePracLesInnerExperimentPage.action";
		form.submit();  
	}
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/practicePlan_deletePracSyllabusInnerExperiment.action";
		form.submit();  
	}
	//数据库传是否
	function LoadFlager2(value){
		var name = "do"+value;
		var form=document.getElementById(name);
		var id="load"+value;
		var value = document.getElementById(id).value;
		if(value=='是'){
			form.action="${pageContext.request.contextPath}/practicePlan_LoadFlagUp2.action?isLoad=1&user.tnum=${user.tnum}";
		}else{
			form.action="${pageContext.request.contextPath}/practicePlan_LoadFlagUp2.action?isLoad=0&user.tnum=${user.tnum}";
		}
...
$(document).ready(function() {
	...
	$(".copySelect").on("change", function() {
		...
		form.action="${pageContext.request.contextPath}/practicePlan_toPracLesInnerExperimentPageCopy.action?syllabusId_Copy=" + this.value;
		form.submit(); 
	});
</script>
</body>
