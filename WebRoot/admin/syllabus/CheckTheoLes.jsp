...
<body>
	<div class="right_cont">
		...
		<div style="padding-left: 24px; padding-top: 10px;">
					...
					<c:forEach items="${haveSyllabusList}" var="haveSyllabus"
						...
						<tr align="center">
							...
							<td><select size="1" id="load${xh.count}"
								style="width: 10em;" onchange="changeLoadFlag('${xh.count}')">
									<c:forEach items="${list}" var="keyword" varStatus="id">
										<option value="${keyword}"
											<c:if test="${keyword eq haveSyllabus.isDow}">selected="selected"</c:if>>${keyword}</option>
									</c:forEach>
							</select></td>
			...
			<div style="border-bottom: #CCCCCC 1 solid;">
				<h4 style="color: red;">
					未编写 “理论课” 大纲：<font style="font-size: 14px;">（注意：复制功能将不复制“教学目标及对应关系”，采用此功能后请再自行填写该项！）</font>
				</h4>
	...
	{ 
		...
		if(value=='是'){
			form.action="${pageContext.request.contextPath}/theoreticalPlan_canLoadFlagUp.action?isLoad=1&user.tnum=${user.tnum}";}
		else{
			form.action="${pageContext.request.contextPath}/theoreticalPlan_canLoadFlagUp.action?isLoad=0&user.tnum=${user.tnum}";}
