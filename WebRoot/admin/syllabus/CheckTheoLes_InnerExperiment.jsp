...
<body>
	<div class="right_cont">
		<div class="title_right">
			<strong>理论课大纲录入——<font style="color: #0000c6;">理论课课内实验</font></strong>
		</div>
		<div class="link">
			<a
				href="${pageContext.request.contextPath}/theoreticalPlan_toCheckTheoLes.action?user.tnum=${user.tnum}"><font
				style="color: #000;">编写理论课大纲</font></a>
		</div>
		<div class="link1">
			<a
				href="${pageContext.request.contextPath}/theoreticalPlan_toCheckTheoLesInnerExperiment.action?user.tnum=${user.tnum}"><font
				style="color: #0000c6;">编写理论课课内实验大纲</font></a>
		</div>
		<div style="padding-left: 24px;">
			<div style="border-bottom: #CCCCCC 1 solid; padding-top: 30px;">
				<h4 style="color: green;">已编写 “理论课课内实验” 大纲：</h4>
			</div>
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<c:forEach items="${haveSyllabusList}" var="haveSyllabus"
						...
						<tr align="center">
							...
							<td nowrap="nowrap"><a
								href="${pageContext.request.contextPath}/ExportWork_exportTheoInnerExperiment.action?syllabusid=${syllabusId[xh.count-1]}&theoexpeid=${haveSyllabus.theoreticalLessonid}"><small>下载</small></a>
							</td>
							<%
							...
							<td><select size="1" id="load${xh.count}"
								style="width: 10em;" onchange="changeLoadFlag1('${xh.count}')">
									<c:forEach items="${list}" var="keyword" varStatus="id">
										<option value="${keyword}"
											<c:if test="${keyword eq haveSyllabus.isDown}">selected="selected"</c:if>>${keyword}</option>
									</c:forEach>
							</select></td>
		...
		<div style="border-bottom: #CCCCCC 1 solid; padding-left: 24px;">
			<h4 style="color: red;">
				未编写 “理论课课内实验” 大纲：<font style="font-size: 14px;">（注意：复制功能将不复制“教学目标及对应关系”，采用此功能后请再自行填写该项！）</font>
			</h4>

		</div>
		<div style="width: 90%; margin-left: 43px; margin-top: 10px;">
			<table class="table table-bordered table-striped table-hover">
				...
				<c:forEach items="${syllabusList}" var="syllabus" varStatus="xh">
					<tr align="center">
						...
						<td nowrap="nowrap"><c:if test="${count[xh.count-1] == -1}">
							...
							<form method="post" id="notDo${xh.count}" name="notDo${xh.count}"
								style="margin: auto;">
								...
								<input type="hidden" name="theoreticalLessonid"
									value="${syllabus.theoreticalLessonid}" /> <input
									type="hidden" name="curriculum.curriculumid"
						...
						<td><select class="copySelect" size="1" style="width: 10em;">
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/theoreticalPlan_toTheoLesPageInnerExperiment.action";
		form.submit();  
	}
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/theoreticalPlan_toupdateTheoLesPageInnerExperiment.action";
		form.submit();  
	}
	...
	{	
		...
		form.action="${pageContext.request.contextPath}/theoreticalPlan_deleteTheoSyllabusInnerExperiment.action";
		form.submit();  
	}
	function changeLoadFlag1(value){
		var name = "do"+value;
		var form=document.getElementById(name);
		var id="load"+value;
		var value = document.getElementById(id).value;
		if(value=='是'){
			form.action="${pageContext.request.contextPath}/theoreticalPlan_canLoadFlagUp1.action?isLoad=1&user.tnum=${user.tnum}";
		}else{
			form.action="${pageContext.request.contextPath}/theoreticalPlan_canLoadFlagUp1.action?isLoad=0&user.tnum=${user.tnum}";
		}
	...
	$(".copySelect").on("change", function() {
		...
		form.action="${pageContext.request.contextPath}/theoreticalPlan_toTheoLesPageCopyInnerExperiment.action?syllabusId_Copy=" + this.value;
		form.submit(); 
	});
</script>
