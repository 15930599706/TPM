...
<body>
	<div class="right_cont">
		...
		<div id="normal_template" style="display: none;">
			<p class="title">
				...
				<button onclick="onDelete({{item._id}}, {{item.discussContentid}})">删除</button>
				&nbsp;&nbsp;&nbsp;&nbsp; 讨论课<span>{{temp.__index+1}}</span> (支撑教学目标：
				<span>{{item.num}}</span>)
			</p>
			...
			<table class="table1">
				...
				<tr>
					<td class="input" colspan="3" style="height: 110px;">讨论课目标:<br>
				...
				<tr>
					<td class="input" style="height: 100px;">讨论课内容:<br> <textarea
				...
				<tr>
					<td class="input" style="height: 100px;">讨论课的实施:<br> <textarea
				...
				<tr>
					<td class="input" style="height: 100px;">讨论课的要求与成绩评定:<br>
		...
		<div id="edit_template" style="display: none;">
			<p class="title-new1">编辑讨论课内容单元</p>
			<div
			...
			<p class="title-new1">
				<button onclick="onUpdate({{item._id}}, {{item.discussContentid}})">更新</button>
	...
	<div
		...
		<table class="table2">
			...
			<tr>
				<td class="input" style="height: 100px;">讨论课内容:<br>
				<textarea id="add_content" style="height: 80px; width: 800px;"></textarea></td>
			</tr>
			<tr>
				<td class="input" style="height: 100px;">讨论课的实施:<br>
...
<script type="text/javascript">
		...
		function onAdd() {
			...
			var url = '${pageContext.request.contextPath}/DiscussContent_adddiscussContent.action?syllabusId=${syllabusId}';
			console.log(item);
			$.get(url, item, function(data) {
			item.discussContentid = data.content;
				list.add(item);
				alert("添加成功");
			});
		}

		function onUpdate(id,discussContentid) {
			var options = {};
			...
			options.discussContentid = discussContentid;
			var url = '${pageContext.request.contextPath}/DiscussContent_updatediscussContent.action?syllabusId=${syllabusId}';
			console.log(options);
		...
		function onDelete(id,discussContentid) {
			var dele={};
			dele.discussContentid = discussContentid;
			var url = '${pageContext.request.contextPath}/DiscussContent_deletediscussContent.action';
			console.log(dele, discussContentid);
