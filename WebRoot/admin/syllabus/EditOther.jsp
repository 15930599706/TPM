...
<body>
	<div class="right_cont">
		...
		<div id="normal_template" style="display: none;">
			<p class="title">
				...
				<button onclick="onDelete({{item._id}},{{item.otherContentid}})">删除</button>
				&nbsp;&nbsp;&nbsp;&nbsp; <span>{{item.name}}</span> <span>{{temp.__index+1}}</span>
			...
			<table class="table1">
				...
				<tr>
					<td class="input" style="height: 100px;">教学内容:<br> <textarea
				...
				<tr>
					<td class="input" style="height: 100px;">课程的实施:<br> <textarea
				...
				<tr>
					<td class="input" style="height: 100px;">课程的进程安排:<br> <textarea
				...
				<tr>
					<td class="input" style="height: 100px;">课程的要求和成绩评定:<br>
		...
		<div id="edit_template" style="display: none;">
			...
			<div
				...
				<tr>
					<td>课程名称：</td>
					<td><input type="text" style="width: 250px;" id="update_name"
						value="{{item.name}}" /></td>
					<td>支撑教学目标：</td>
					<td><input type="text" style="width: 110px;" id="update_num"
						value="{{item.num}}" /></td>
				</tr>
			</div>
			<table class="table2">
			...
			<p class="title-new1">
				<button onclick="onUpdate({{item._id}},{{item.otherContentid}})">更新</button>
	...
	<div
		...
		class="new">
		<p class="title-new">新建课程内容单元</p>
		<div
			style="width: 800px; height: 40px; font-size: 15px; font-weight: bold;">
			<tr>

				<td>课程名称：</td>
				<td><input id="add_name" type="text" style="width: 110px;" />
				</td>
				<td>支撑教学目标：</td>
...
<script type="text/javascript">
		...
		function onAdd() {
			...
			var url = '${pageContext.request.contextPath}/OtherContent_addOtherContent.action?syllabusId=${syllabusId}';
			console.log(item);
			$.post(url, item, function(data) {
			item.otherContentid = data.content;
				list.add(item);
				alert("添加成功");
			});
		}

		function onUpdate(id,otherContentid) {
			var options = {};
			...
			options.otherContentid = otherContentid;
			var url = '${pageContext.request.contextPath}/OtherContent_updateOtherContent.action?syllabusId=${syllabusId}';
			console.log(options);
		...
		function onDelete(id,otherContentid) {
			var dele={};
			dele.otherContentid = otherContentid;
			var url = '${pageContext.request.contextPath}/OtherContent_deleteOtherContent.action';
