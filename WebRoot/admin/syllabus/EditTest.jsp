	...
	<div class="right_cont">
		...
		<div id="normal_template" style="display: none;">
			<p class="title">
				...
				<button
					onclick="onDelete({{item._id}}, {{item.experimentContentid}})">删除</button>
				&nbsp;&nbsp;&nbsp;&nbsp; 实验<span>{{temp.__index+1}}</span> (支撑教学目标：
				<span>{{item.num}}</span>)
			</p>
			...
			<table class="table1">
				...
				<tr>
					<td class="input" style="height: 100px;">主要内容:<br> <textarea
		...
		<div id="edit_template" style="display: none;">
			...
			<p class="title-new1">
				<button
					onclick="onUpdate({{item._id}}, {{item.experimentContentid}})">更新</button>
...
<script type="text/javascript">
		// 创建实例
		var init_data=${experimentContent};
		// 创建实例
		...
		function onAdd() {
			...
			var url = '${pageContext.request.contextPath}/ExperimentContent_addExperimentContent.action?syllabusId=${syllabusId}';
			console.log(item);
			
			$.get(url, item, function(data) {
			item.experimentContentid = data.content;
				list.add(item);
				alert("添加成功");
			});
		}

		function onUpdate(id,experimentContentid ) {
			var options = {};
			...
			options.experimentContentid = experimentContentid;

			var url = '${pageContext.request.contextPath}/ExperimentContent_updataExperimentContent.action?syllabusId=${syllabusId}';
			console.log(options);
		...
		function onDelete(id, experimentContentid) {
			var dele={};
			dele.experimentContentid = experimentContentid;
			var url = '${pageContext.request.contextPath}/ExperimentContent_deleteExperimentContent.action';
			console.log(dele);
