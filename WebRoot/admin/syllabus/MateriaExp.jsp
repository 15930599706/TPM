	...
	<div class="right_cont">
		...
		<div id="normal_template" style="display: none;">
			<p class="title">
				...
				<button
					onclick="onDelete({{item._id}}, {{item.practiceBooks_InnerExperimentid}})">删除</button>
		...
		<div id="edit_template" style="display: none;">
			...
			<p class="title-new1">
				<button
					onclick="onUpdate({{item._id}}, {{item.practiceBooks_InnerExperimentid}})">更新</button>
...
<script type="text/javascript">
		...
		function onAdd() {
			...
			var url = '${pageContext.request.contextPath}/PracticeBookInnerExperiment_addexperimentBooks.action?syllabusId=${syllabusId}';
			console.log(item);
			$.post(url, item, function(data) {
			item.practiceBooks_InnerExperimentid = data.content;
				list.add(item);
				alert("添加成功");
			});
		}

		function onUpdate(id,practiceBooks_InnerExperimentid) {
			var options = {};
			options.name = document.getElementById("update_name").value;
			options.practiceBooks_InnerExperimentid = practiceBooks_InnerExperimentid;

			var url = '${pageContext.request.contextPath}/PracticeBookInnerExperiment_updateexperimentBooks.action?syllabusId=${syllabusId}';
			console.log(options);
		...
		function onDelete(id,practiceBooks_InnerExperimentid) {
			var dele={};
			dele.practiceBooks_InnerExperimentid = practiceBooks_InnerExperimentid;
			var url = '${pageContext.request.contextPath}/PracticeBookInnerExperiment_deleteexperimentBooks.action';
			console.log(dele);
