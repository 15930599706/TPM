	...
	<div class="right_cont">
		...
		<div id="normal_template" style="display: none;">
			<p class="title">
				...
				<button
					onclick="onDelete({{item._id}}, {{item.textBooks_InnerExperimentid}})">删除</button>
		...
		<div id="edit_template" style="display: none;">
			...
			<p class="title-new1">
				<button
					onclick="onUpdate({{item._id}}, {{item.textBooks_InnerExperimentid}})">更新</button>
...
<script type="text/javascript">
		...
		function onAdd() {
			...
			var url = '${pageContext.request.contextPath}/TextBooksInnerExperimentAction_addTextBooksInnerExperiment.action?syllabusId=${syllabusId}';
			console.log(item);
			$.post(url, item, function(data) {
			item.textBooks_InnerExperimentid = data.content;
				list.add(item);
				alert("添加成功");
			});
		}

		function onUpdate(id,textBooks_InnerExperimentid) {
			var options = {};
			options.name = document.getElementById("update_name").value;
			options.textBooks_InnerExperimentid = textBooks_InnerExperimentid;

			var url = '${pageContext.request.contextPath}/TextBooksInnerExperimentAction_updateTextBooksInnerExperimen.action?syllabusId=${syllabusId}';
			console.log(options);
		...
		function onDelete(id,textBooks_InnerExperimentid) {
			var dele={};
			dele.textBooks_InnerExperimentid = textBooks_InnerExperimentid;
			var url = '${pageContext.request.contextPath}/TextBooksInnerExperimentAction_deleteTextBooksInnerExperimens.action';
			console.log(dele);
