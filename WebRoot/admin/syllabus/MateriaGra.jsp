...
<body>
	<div class="right_cont">
		<div class="title_right">
			<strong>毕业设计教材信息</strong>
		</div>
		<div id="box9">
			<a
				href="${pageContext.request.contextPath}/practicePlan_toHavePracLesGraduationProjectPage.action?syllabusId=${syllabusId}&practiceLessonid=${newpracticeLesson.practiceLessonid}">返回录入其他内容</a>
		</div>
		...
		<div id="normal_template" style="display: none;">
			<p class="title">
				...
				<button onclick="onDelete({{item._id}}, {{item.practiceBookid}})">删除</button>
		...
		<div id="edit_template" style="display: none;">
			...
			<p class="title-new1">
				<button onclick="onUpdate({{item._id}}, {{item.practiceBookid}})">更新</button>
...
<script type="text/javascript">
	...
	/* 	var textBooks = document.getElementById('ta').value; 
		var init_data =JSON.parse(textBooks); */
		// 创建实例
		var list = new List(container, ${practiceBook});
		// 指定模板
		...
		function onAdd() {
			...
			var url = '${pageContext.request.contextPath}/PracticeBook_addpracticeBook.action?syllabusId=${syllabusId}';
			console.log(item);
			$.post(url, item, function(data) {
			item.practiceBookid = data.content;
				list.add(item);
				alert("添加成功");
			});
		}

		function onUpdate(id,practiceBookid) {
			var options = {};
			options.name = document.getElementById("update_name").value;
			options.practiceBookid = practiceBookid;

			var url = '${pageContext.request.contextPath}/PracticeBook_updatepracticeBook.action?syllabusId=${syllabusId}';
			console.log(options);
		...
		function onDelete(id,practiceBookid) {
			var dele={};
			dele.practiceBookid = practiceBookid;
			var url = '${pageContext.request.contextPath}/PracticeBook_deletepracticeBook.action';
			console.log(dele);
