	...
	<div class="right_cont">
		...
		<div id="normal_template" style="display: none;">
			<p class="title">
				...
				<button onclick="onDelete({{item._id}},{{item.textBooksid}})">删除</button>
		...
		<div id="edit_template" style="display: none;">
			...
			<p class="title-new1">
				<button onclick="onUpdate({{item._id}},{{item.textBooksid}})">更新</button>
				<button onclick="changeStatus({{item._id}}, 'normal')">取消</button>
			</p>
		</div>

		<div
			style="background-color: #ffecc5; width: 810px; margin-left: 24px; margin-top: 10px;">
			<p class="title-new">添加教材及相关资源</p>
			<div
				style="width: 760px; height: 40px; font-size: 15px; font-weight: bold; padding-top: 8px;">
				<tr>
					<td>资源名称：</td>
...
<script type="text/javascript">
		...
		function onAdd() {
			...
			var url = '${pageContext.request.contextPath}/TextBooks_addtextBooks.action?syllabusId=${syllabusId}';
			console.log(item);
			$.post(url, item, function(data) {
			item.textBooksid = data.content;
				list.add(item);
				alert("添加成功");
			});
		}
		
		function onUpdate(id,textBooksid) {
			var options = {};
			options.name = document.getElementById("update_name").value;
			options.textBooksid = textBooksid;
			var url = '${pageContext.request.contextPath}/TextBooks_updatetextBooks.action?syllabusId=${syllabusId}';
			console.log(options);
		...
		function onDelete(id,textBooksid) {
			var dele = {};
			dele.textBooksid = textBooksid;
			var url = '${pageContext.request.contextPath}/TextBooks_deletetextBooks.action';
			console.log(dele);
