...
<script language="javascript">
	   ...
	   try{
		...
		function selectSubmit()
		{
			...
			form.action="${pageContext.request.contextPath}/user_tosyszrglPage.action?tnum=${user.tnum}";
			form.submit();   
		}	
		
				
		function tjsyszr_check(){	//生成学院培养方案学院id提交
			var shiyanshiid = document.getElementById("shiyanshi").value;
			if(shiyanshiid != "-1")	//限定有选择时才会提交表单
			{
				var form=document.getElementById("dlzh");
				form.action="${pageContext.request.contextPath}/user_totjsyszrPage.action?tnum=${user.tnum}";
				form.submit(); 
			} 
			else
			{
...
<body onload="xueyuanSelect()">
	...
	<div class="right_cont">
		<div class="title_right">
			<strong>实验室主任管理</strong>
		</div>

		<div id="box1">
			<a href="#" target="MainFrame" onclick="tjsyszr_check()">添加实验室主任</a>
		</div>
		<c:if test="${user.adminlevel == 5 }">
			<a href="${pageContext.request.contextPath}/user_tosysrzAdmin.action"
				target="MainFrame">查看修改日志</a>
		</c:if>
		<form method="post"
			action="${pageContext.request.contextPath}/user_tosyszrglPage.action?tnum=${user.tnum}"
			id="dlzh">
			<div id="box2">
				<td>学院：</td>
				<c:if test="${user.adminlevel == 5 }">
		...
		<div style="width: 70%; margin-left: 25px;">
			<form
				action="${pageContext.request.contextPath}/user_tosyszrglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&experiment.experimentid=${newuser.experiment.experimentid}"
					...
					<c:forEach items="${pageBean.userlist }" var="users">
						<tr align="center" class="even">
							...
							<td nowrap="nowrap"><c:if
									...
									<a
										href="${pageContext.request.contextPath}/user_toeasyexperimentAdmin.action?modifyid=${user.tnum}&tnum=${users.tnum }"
						...
						<tfoot>
							<tr align="center">
								<th nowrap="nowrap" colspan="9"><c:if
										...
										<a
											href="${pageContext.request.contextPath}/user_tosyszrglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&experiment.experimentid=${newuser.experiment.experimentid}"><small>首页</small></a>&nbsp;&nbsp;&nbsp;
									<a
											href="${pageContext.request.contextPath}/user_tosyszrglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&experiment.experimentid=${newuser.experiment.experimentid}&currentpage=${currentpage-1 }"><small>上一页</small></a>
									</c:if>&nbsp;&nbsp;&nbsp; <small>共${pageBean.totalCount }条记录,共计${pageBean.totalPage }页,当前第${pageBean.currentpage }页</small>&nbsp;&nbsp;&nbsp;
									<c:if test="${pageBean.currentpage != pageBean.totalPage }">
										<a
											href="${pageContext.request.contextPath}/user_tosyszrglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&experiment.experimentid=${newuser.experiment.experimentid}&currentpage=${currentpage+1 }"><small>下一页</small></a>&nbsp;&nbsp;&nbsp;
									<a
											href="${pageContext.request.contextPath}/user_tosyszrglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&experiment.experimentid=${newuser.experiment.experimentid}&currentpage=${pageBean.totalPage }"><small>尾页</small></a>
