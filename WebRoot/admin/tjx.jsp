	   ...
	   try{
		...
		function selectSubmit()
		{
			...
			var xiid = document.getElementById("xi").value;
				...
				form.action="${pageContext.request.contextPath}/user_totjxPage.action?tnum=${user.tnum}";
				form.submit();   
		}	
	</script>
...
<body
	...
	<div class="right_cont">
		<div class="title_right">
			<strong>添加专业管理员</strong>
		</div>


		<form method="post"
			action="${pageContext.request.contextPath}/user_totjxPage.action?tnum=${user.tnum}"
			id="dlzh">
			<div id="box2">
				<td>学院：</td>
		...
		<div style="width: 70%; margin-left: 24px;">
			<form
				action="${pageContext.request.contextPath}/user_totjxPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.cepartmentid=${newuser.department.departmentid}"
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<c:forEach items="${pageBean.userlist }" var="users">
						<tr align="center" class="even">
							...
							<td nowrap="nowrap"><small>普通老师</small></td>
							<td nowrap="nowrap"><c:if
									test="${not empty users.department.departmentCname && users.department.departmentCname != '其他'}">
									<a
										href="${pageContext.request.contextPath}/user_todepartmentadmin.action?tnum=${users.tnum }"
										onclick="if (confirm('您确定要将该用户权限设置成专业管理员吗？')) return true; else return false;"><small>添加为专业管理员</small></a>
								</c:if></td>
						</tr>
					</c:forEach>
						...
						<tfoot>
							<tr align="center">
								<th nowrap="nowrap" colspan="9"><c:if
										...
										<a
											href="${pageContext.request.contextPath}/user_totjxPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.departmentid=${newuser.department.departmentid}"><small>首页</small></a>&nbsp;&nbsp;&nbsp;
									<a
											href="${pageContext.request.contextPath}/user_totjxPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.departmentid=${newuser.department.departmentid}&currentpage=${currentpage-1 }"><small>上一页</small></a>
									</c:if>&nbsp;&nbsp;&nbsp; <small>共${pageBean.totalCount }条记录,共计${pageBean.totalPage }页,当前第${pageBean.currentpage }页</small>&nbsp;&nbsp;&nbsp;
									<c:if test="${pageBean.currentpage != pageBean.totalPage }">
										<a
											href="${pageContext.request.contextPath}/user_totjxPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.departmentid=${newuser.department.departmentid}&currentpage=${currentpage+1 }"><small>下一页</small></a>&nbsp;&nbsp;&nbsp;
									<a
											href="${pageContext.request.contextPath}/user_totjxPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.departmentid=${newuser.department.departmentid}&currentpage=${pageBean.totalPage }"><small>尾页</small></a>
