	   ...
	   try{
		...
		function selectSubmit()
		{
			...
			var xiid = document.getElementById("xi").value;
				...
				form.action="${pageContext.request.contextPath}/user_toxyglPage.action?tnum=${user.tnum}";
				form.submit();   
		}	
	</script>
...
<body
	...
	<div class="right_cont">
		<div class="title_right">
			<strong>学院管理员管理</strong>
		</div>

		<div id="box1">
			<c:if test="${user.adminlevel == 5 }">
				<a
					href="${pageContext.request.contextPath}/user_totjxyPage.action?tnum=${user.tnum }&username=${nuser.username}&college.collegeid=${nuser.college.collegeid}&department.departmentid=${nuser.department.departmentid}"
					target="MainFrame">添加学院管理员</a>
			</c:if>
			<c:if test="${user.adminlevel == 3 }">
				<a
					href="${pageContext.request.contextPath}/user_totjxyPage.action?tnum=${user.tnum }&username=${nuser.username}&college.collegeid=${user.college.collegeid}&department.departmentid=${nuser.department.departmentid}"
					target="MainFrame">添加学院管理员</a>
			</c:if>
		</div>
		<form method="post"
			action="${pageContext.request.contextPath}/user_toxyglPage.action?tnum=${user.tnum}"
			id="dlzh">
			<div id="box2">
				<td>学院：</td>
				<c:if test="${user.adminlevel == 5 }">
		...
		<div style="width: 70%; margin-left: 25px;">
			<form
				action="${pageContext.request.contextPath}/user_toxyglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.cepartmentid=${newuser.department.departmentid}"
						...
						<tfoot>
							<tr align="center">
								<th nowrap="nowrap" colspan="9"><c:if
										...
										<a
											href="${pageContext.request.contextPath}/user_toxyglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.departmentid=${newuser.department.departmentid}"><small>首页</small></a>&nbsp;&nbsp;&nbsp;
									<a
											href="${pageContext.request.contextPath}/user_toxyglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.departmentid=${newuser.department.departmentid}&currentpage=${currentpage-1 }"><small>上一页</small></a>
									</c:if>&nbsp;&nbsp;&nbsp; <small>共${pageBean.totalCount }条记录,共计${pageBean.totalPage }页,当前第${pageBean.currentpage }页</small>&nbsp;&nbsp;&nbsp;
									<c:if test="${pageBean.currentpage != pageBean.totalPage }">
										<a
											href="${pageContext.request.contextPath}/user_toxyglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.departmentid=${newuser.department.departmentid}&currentpage=${currentpage+1 }"><small>下一页</small></a>&nbsp;&nbsp;&nbsp;
									<a
											href="${pageContext.request.contextPath}/user_toxyglPage.action?tnum=${user.tnum}&username=${newuser.username}&college.collegeid=${newuser.college.collegeid}&department.departmentid=${newuser.department.departmentid}&currentpage=${pageBean.totalPage }"><small>尾页</small></a>
